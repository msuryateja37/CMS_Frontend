...
const IncidentRow: React.FC<IncidentRowProps> = ({ refId, title, type, severity, status, date, assigned, loc }) => {
    ...
    const severityColors: Record<string, string> = {
        ...
        'Low': 'bg-subtle-green text-green border border-green/20',
    };

    return (
        <tr className="hover:bg-subtle-grey transition-all group cursor-pointer border-b border-semi-subtle-grey">
            <td className="px-6 py-5">
                <input type="checkbox" className="w-4 h-4 rounded border-semi-subtle-grey text-green focus:ring-green cursor-pointer" />
            </td>
            <td className="px-6 py-5">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-dark-green text-sm">{refId}</span>
                </div>
            </td>
            <td className="px-6 py-5">
                <div className="flex flex-col">
                    <span className="text-sm font-bold text-dark-green line-clamp-1">{title}</span>
