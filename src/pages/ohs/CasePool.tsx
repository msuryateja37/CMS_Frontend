...
const CasePool: React.FC = () => {
  ...
  const isForwardedCase = (c: Case) => c.status === 'FORWARDED_TO_OHS_AND_HR';
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {cases.map((c) => {
                ...
                const hrStatus = (c as any).hrStatus as string | undefined;
                const hrAssignedTo = (c as any).hrAssignedTo as { name: string } | undefined;
                return (
                  <tr key={c.id} className={`border-t border-gray-100 hover:bg-gray-50 ${forwarded ? 'bg-purple-50/30' : ''}`}>
                    ...
                    <td className="px-5 py-3">{(c as any).building?.province?.name ?? c.building?.province?.name ?? '—'}</td>
                    <td className="px-5 py-3">
                      <Pill
                    ...
                    <td className="px-5 py-3">
                      {forwarded && hrStatus ? (
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                          hrStatus === 'HR_APPROVED' ? 'bg-green-100 text-green-700'
