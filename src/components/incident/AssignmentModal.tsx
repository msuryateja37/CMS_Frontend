...
// Map category selection to backend role names
const PRACTITIONER_CATEGORIES = [
    { value: 'OHS_PRACTITIONER', label: 'OHS' },
    { value: 'SECURITY_PRACTITIONER', label: 'Security' },
];

const AssignmentModal: React.FC<AssignmentModalProps> = ({ isOpen, onClose, incidentId, provinceId, onSuccess }) => {
    ...
    // Fetch practitioners by role + province when category changes
    useEffect(() => {
        setSelectedUserId('');
        if (provinceId && selectedCategory) {
            fetchPractitioners(provinceId, selectedCategory);
        } else {
            setUsers([]);
        }
    }, [provinceId, selectedCategory]);

    const fetchPractitioners = async (pId: string, role: string) => {
        try {
            setIsLoadingUsers(true);
            const data = await userService.listFiltered({ provinceId: pId, role });
            setUsers(data);
        } catch (err) {
        ...
        >
            <div className="space-y-4">
                ...
                <Select
                    ...
                    placeholder="Choose OHS or Security..."
                />

                {/* Practitioner List with Ticket Counts */}
                {selectedCategory && (
                    <div>
                        ...
                        {isLoadingUsers ? (
                            ...
                            <p className="text-xs text-red-500 py-3">
                                No practitioners found for this category in the case's area.
                            </p>
                        ) : (
